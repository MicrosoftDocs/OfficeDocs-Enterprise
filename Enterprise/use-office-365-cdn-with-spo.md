...
You can use Office 365's built-in public and private Content Delivery Networks (CDNs) to host static assets to provide better performance for your SharePoint Online pages. CDNs improve performance by caching static assets closer to the browsers requesting them, which helps to speed up downloads and reduce latency. Static assets are files that don't change very often, like images, video and audio, style sheets, fonts, and JavaScript files.

Office 365 uses two distinct classes of CDN: **public** and **private**. Both public and private CDNs allow you to host static assets in multiple locations, or _origins_, and are included as part of your SharePoint Online subscription.
...
Public CDNs are hosted by CDN providers like Akamai, Verizon and Microsoft Azure. Content on a public CDN is accessible anonymously to anyone who has the URLs to hosted assets. Because access to content in a public CDN is anonymous, you should only use them to cache non-sensitive generic content such as javascript files, icons and images.
...
To set up a CDN for your Office 365 tenant, you follow these basic steps:
...
- Assets in the private CDN must be referred by the SharePoint Online tenant.
...
## Auto-rewriting of CDN URLs

To help organizations take advantage of the Office 365 CDN capabilities without having to update their existing portals, the SharePoint Publishing feature has been updated to automatically rewrite URLs of assets stored in CDN origins to their CDN equivalents so that assets are served from the CDN service instead of SharePoint. You do not have to hardcode URLs for CDN assets.
...
The CDN service in Office 365 provides an auto-rewrite feature that automatically generates CDN URLs for assets in an origin, and returns the rewritten URL when the asset is requested. You do not have to hardcode URLs for CDN assets.
